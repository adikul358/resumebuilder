...
const Dashboard = () => {
    ...
    useEffect(() => {
        (async function () {
            let res = await fetch(`${process.env.API_HOST_URL}/metadata`, { method: "GET" })
            let t = await res.json()
            Object.keys(t).forEach(v => t[v] = t[v] == null ? "" : t[v])
            dispatchMetadata({ type: "SET", payload: t })
            let res2 = await fetch(`${process.env.API_HOST_URL}/resumes`, { method: "GET" })
    ...
    const submitForm = async (e) => {
        ...
        const res = await fetch(`${process.env.API_HOST_URL}/metadata`, {
            method: "POST",
